...
angular.module('sipkpApp')
    .controller('UserGrosirBulananCtrl', function ($scope, Restangular, socket, blockUI, $timeout) {
        ...
        function grMonth(arrRange, arrData, tanggal) {
            var temp = {
                sapi: [],
                kerbau: [],
                kambing: [],
        ...
        function getGrosir(tanggal, komoditi) {
            ...
            Restangular.all('grosirs').customGETLIST().then(function (datas) {
                $timeout(function () {
                    $scope.range = range;
                    $scope.grb = _.map(grMonth(range, datas, tanggal));
                    $scope.grblabels = range;
                    $scope.grbseries = [komoditi];
                    $scope.grbdata = [graph(range, datas, tanggal, komoditi)];

                    socket.syncUpdates('grosir', datas, function (event, item, array) {
                        $scope.range = range;
                        $scope.grb = _.map(grMonth(range, array, tanggal));
                        $scope.grblabels = range;
                        $scope.grbseries = [komoditi];
                        $scope.grbdata = [graph(range, array, tanggal, komoditi)];
                    });
                    blockUI.stop();
                }, 1000);
            });
