...
angular.module('sipkpApp')
    .controller('UserEceranCreateCtrl', function ($scope, Restangular, $alert) {

        $scope.data = {};
        $scope.data.tanggal = Date.now();
        $scope.data.sapihasdalam = 0;
        $scope.data.sapibistik = 0;
        $scope.data.sapimurni = 0;
        $scope.data.kerbau = 0;
        ...
        $scope.submit = function (form) {
            ...
            if (form.$valid) {
                Restangular.all('ecerans').customPOST({
                        tanggal: $scope.data.tanggal,
                        sender: $scope.getCurrentUser().telp,
                        kabkota: $scope.getCurrentUser().kabkota,
                        sapihasdalam: $scope.data.sapihasdalam,
                        sapibistik: $scope.data.sapibistik,
                        sapimurni: $scope.data.sapimurni,
